...
include 'C:\xampp\htdocs\web1\connection.php';
session_start();
$id = $_SESSION['id'];
...
if(isset($_POST['update_profile'])){
   ...
   mysqli_query($con, "UPDATE `register` SET first_name = '$first_name', email = '$email' WHERE id = '$id'") or die('query failed');
   ...
   if(!empty($update_pass) || !empty($new_pass) || !empty($confirm_pass)){
      if($update_pass != $password){
         ...
         mysqli_query($con, "UPDATE `register` SET password = '$confirm_pass' WHERE id = '$id'") or die('query failed');
   ...
   if(!empty($update_image)){
      if($update_image_size > 2000000){
         ...
         $image_update_query = mysqli_query($con, "UPDATE `register` SET image = '$update_image' WHERE id = '$id'") or die('query failed');
...
<div class="update-profile">
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <div class="flex">
         ...
         <div class="inputBox">
            <input type="hidden" name="password" value="<?php echo $fetch['password']; ?>">
